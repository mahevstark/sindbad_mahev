...
const SearchResult = (props) => {
    ...
    const {search_text} = props?.route?.params;
    ...
    const loadOrders = async () =>{
        const dbData = { lang: "en" ,token:user?.token,per_page:99,id:search_text}

        const { isError, data } = await doPost(dbData, "get_search_products_data");
        console.log(dbData)
        console.log(urls.API + "get_search_products_data");
        if (isError) {
                        ...
                        <View style={{ flex: 1 }}>
                            <View style={{ flexDirection: 'row', width: '90%', alignSelf: 'center', justifyContent: 'space-between', marginTop: Platform.OS == 'android' ? 25 : 5, alignItems: 'center' }}>
                                ...
                                <View style={{ flexDirection: 'row', alignItems: 'center', }}>
                                    ...
                                    <Text style={{ marginLeft: 10, fontSize: 21, fontFamily: 'LRe', color: '#FA6443' }}>{search_text ?? "Products"}</Text>
                                </View>
                            </View>
                            <View
                                style={{
                            ...
                            >
                                ...
                                >
                                    {( !prods?.length || prods?.length==0)&& <View style={{width:"100%",alignSelf:"center",marginVertical:20,alignItems:"center"}}><Text>No products in this search result</Text></View>}
